...
async function LoadDataBase(naze, m) {
	try {
		...
		let limitUser = user
			? user.vip
				? global.limit.vip
				: prem.checkPremiumUser(m.sender, premium)
		...
		if (m.isGroup) {
			...
			if (group) {
				...
				if (!("antitoxic" in group)) group.antitoxic = true
	...
	try {
		...
		if (type === "interactiveResponseMessage" && m.quoted && m.quoted.fromMe) {
			let apb = await generateWAMessage(
				m.chat,
				{ text: JSON.parse(m.msg.nativeFlowResponseMessage.paramsJson).id, mentions: m.mentionedJid },
				{
		...
		if (global.db.set && global.db.set[botNumber] && global.db.set[botNumber].readsw) {
			if (msg.key.remoteJid === "status@broadcast") {
				...
				if (/protocolMessage/i.test(type))
					naze.sendFromOwner(
						global.owner,
						"Status dari @" + msg.key.participant.split("@")[0] + " Telah dihapus",
						msg,
				...
				if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
					let keke =
						type == "extendedTextMessage"
							? `Story Teks Berisi : ${msg.message.extendedTextMessage.text ? msg.message.extendedTextMessage.text : ""}`
							: type == "imageMessage"
...
async function Solving(naze, store) {
	...
	naze.getName = (jid, withoutContact = false) => {
		...
		if (id.endsWith("@g.us")) {
			...
			return Promise.resolve(
				groupInfo.name || groupInfo.subject || PhoneNumber("+" + id.replace("@g.us", "")).getNumber("international"),
			)
		} else {
		...
		async function getFileUrl(res, mime) {
			if (mime && mime.includes("gif")) {
				...
				return naze.sendMessage(
					jid,
					{ document: res.data, mimetype: "application/pdf", caption: caption, ...options },
					{ quoted },
				)
				...
				return naze.sendMessage(
					jid,
					{ video: res.data, caption: caption, mimetype: "video/mp4", ...options },
					{ quoted },
				)
	...
	naze.sendTextMentions = async (jid, text, quoted, options = {}) =>
		naze.sendMessage(
			jid,
	...
	if (m.key) {
		...
		if (m.isGroup) {
			...
			m.admins = m.metadata.participants.reduce(
				(a, b) => (b.admin ? a.push({ id: b.id, admin: b.admin }) : [...a]) && a,
				[],
			)
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.quoted.isBot = m.quoted.id
				? m.quoted.id.startsWith("BAE5") || m.quoted.id.startsWith("HSK") || m.quoted.id.length === 22
				: false
