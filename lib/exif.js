...
async function writeExif(media, data) {
	...
	const wMedia = /webp/.test(anu.mime)
		? media
		: /jpeg|jpg|png/.test(anu.mime)
	...
	if (data) {
		...
		const wr = {
			a: global.author ? global.author : "naze-dev",
			b: data.packname ? data.packname : global.packname ? global.packname : "Bot WhatsApp",
			c: data.author ? data.author : global.author ? global.author : "Nazedev",
			d: data.categories ? data.categories : [""],
			e: data.isAvatar ? data.isAvatar : 0,
		}
		const json = {
